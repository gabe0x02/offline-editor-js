...
define([
    ...
    function(query,request,Point,Extent,SpatialReference,TileInfo,LOD,Base64Utils,TilingScheme,FileSaver){
        "use strict";
        var TilesCore = function(){
            ...
            this._getTiles = function(image,imageType,url,tileid,store){
                this._retrieveTile(url, store, function(success, offlineTile) { 
            ...
            this._retrieveTile = function(url, store, callback) {
              store.retrieve(url, function(success, offlineTile) {
                callback(success, offlineTile);
              });
            };


            var downloading = {};
            this._downloading = function(url) {
              url = url.split("?")[0];
              if(downloading[url]) {
                return true;
              } else {
            ...
            this._createCellsForOffline = function(context,minLevel,maxLevel,extent,callback){
                ...
                for(var level=minLevel; level<=maxLevel; level++)
                {
                    ...
                    if( cells.length > MAX_OFFLINE_TILES && level !== maxLevel)
                    {
                        console.error("Max Offline Tiles", MAX_OFFLINE_TILES, level);
                        break;
                    }
                }
                callback(cells);
